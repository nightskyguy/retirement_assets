...
| `sweep_golden.js` | Characterization goldens for the **two** strategy enumerations — Monte Carlo's `buildVariations()` and the Optimizer's inline sweep in `_runOptimizerNow()`. Data only, dual-mode export, read by `optimizer_core.test.js`. Recorded before the P35 PR 2 extraction so it can be shown to preserve behavior. |
