...
## Session: 2026-07-29 (worktree context-e73361, branch worktrees/markdown-changelog-rendering-a11a86) — P25 markdown docs render (v11.13c2)
...
- VERIFIED: node doclinks 16/16, optimizer_core 148/148, taxPaymentPlanner 27/27 (engine untouched, so those two had to stay put and did). Browser on localhost:8768 — local path leaves all 8 `.md` hrefs alone (`isRendered()` false); forced-rendered path rewrites exactly those 8 of 35 links and nothing else; in-page suite 242/242, Documentation tab 🟢, console clean apart from the 4 known fixtures; title/first-`<li>`/stat-version all read 11.13c2, 5 entries with 11.1370 dropped; 59 changelog anchors, all unique.
