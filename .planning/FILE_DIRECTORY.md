...
| `feedback.js` | The **Send feedback** dialog (P131). Loaded in `<head>`, not deferred, so it records page errors from the start; builds nothing until `FeedbackWidget.open()`. Decides what a report carries (settings through the page's safe-key list, the full plan link and a screenshot only when ticked), captures the page with a pinned, hash-checked `html2canvas-pro`, runs the Turnstile bot check and posts to the Worker in `.feedback-worker/`. Pure half exported for node. |
...
| `feedback.tests.js` | `node`-run test suite for Send feedback: `feedback.js`'s pure half, the safe/personal split of every share-link key in `optimizer_ui.js`, and the Worker's `logic.cjs` driven through `handle()` with stand-ins for Cloudflare. Its runner awaits each test. |
...
| `.test_harnesses/rmdbasis_harness.js` | `node` investigative script for P84 — characterizes the RMD basis before and after `P84l`, and scores whether the fix moved what the characterization predicted. Report: `research/RMD_BASIS.md`. |
| `.test_harnesses/convtiming_harness.js` | `node` investigative script for P85 — front-load vs level vs back-load Roth conversion schedules at equal lifetime gross. Report: `research/CONVERSION_TIMING.md`. |
...
| `.test_harnesses/unifiedconv_harness.js` | `node` investigative script for P28 — models every voluntary IRA withdrawal as a Roth conversion across a 630-simulation grid. Findings live in `research/CONVERSION_ROUTING.md`. |
| `research/README.md` | Index of every research report: one row per report with a link and a one or two sentence summary of what it covers and what it found. Also carries the three rules for adding a new one - name the file for its subject rather than the phase, define every code before first use, and add the row in the same commit. |
| `research/CONVERSION_ROUTING.md` | The P28 reference write-up: tables, reasoning, and the seven headline findings the harness produced. |
...
| `.github/ISSUE_TEMPLATE/` | `feedback.yml`, the public GitHub issue form the Send feedback dialog links to (it fills the `what`, `version` and `browser` fields by id; `feedback.tests.js` checks they exist), and `config.yml`, which points the New issue page at the private Feedback button. Takes effect once on `main`. |
| `.feedback-worker/` | The Cloudflare Worker behind Send feedback (P131), served at `feedback.netcitizen.us`: `src/logic.cjs` (every check, the daily count and the email), `src/index.js` (the Cloudflare entry point and the `DailyCounter` Durable Object), `wrangler.jsonc`, `local-server.cjs` (the same `handle()` under plain node, writing `.eml` files to a git-ignored `.outbox/` instead of mailing, for testing the page with `?fbdev` and nothing installed), and a README with the one-time setup. Dot-prefixed so Jekyll never publishes it. Holds no addresses or secrets: those are Worker secrets. |
