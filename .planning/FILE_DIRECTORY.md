...
| `.githooks/md-html-scan.js` | Blocks raw HTML in markdown that hides the rest of the file. A bare `<select>` cost 4,430 lines of VS Code preview in `task_plan.md`; GitHub sanitizes the tag away, so only an unsanitized preview shows the break. Denylists the ~13 elements whose content model excludes flow content; `<a>`/`<b>`/`<details>` and friends pass. Run standalone on any file: `node .githooks/md-html-scan.js <file.md>`. |
