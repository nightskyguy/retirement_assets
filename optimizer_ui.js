...
function updateTable(log) {
    ...
    const tooltips = {
        ...
        'IRMAATier': `IRMAA tier (Tier 1 to Tier 5) derived from MAGI 2 years ago. Shows -none- below the first threshold, where there is no surcharge, and until a spouse reaches ${medAge} (Medicare age).`,
        'Medicare': `Base cost for Medicare Parts B + D for spouses ${medAge}+. Grows on a published Medicare path that starts near 6.6% a year and eases toward 3.8%, not at your inflation rate. Illustration only - not deducted from spendable income; assumed inside the spend goal. Excludes IRMAA (separate column).`,
