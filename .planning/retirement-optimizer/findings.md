...
## The baseline/proportional strategy family cannot fund its own tax bill once the taxable accounts run dry (2026-08-05, v11.1447)

**Verdict: a defect, not the strategy working as designed.** Pre-existing and byte-identical before
P35 PR 3c (`d68d27f`); that PR only made it visible, because a lapsed ACA plan now falls through to
this code path. Recorded here only. **No engine change made in this pass** — `propwd`, `fixed` and
`gk` are shipped strategies and any fix moves numbers on every saved plan that uses them.

**Repro** (`CAP_BASE`, `optimizer_core.test.js:783`), overrides
...
`yr.additionalSpendNeeded` (`optimizer_core.js:1235`) is `targetSpend + IRMAA - possibleIncome`,
and `possibleIncome` (`:1189`) is **gross** income — SS plus taxable RMD, pre-tax. So the primary
...
(`:1709`, `taxengine.js:1122-1125` calls `nr` "the EFFECTIVE rate at this bracket's top"), not the
marginal rate the draw actually lands in. Both understatements are by design a first approximation;
the gap fill (`:1466`) and the third pass (`:1584`) exist to correct them. They cannot, here.

Measured ledger, 2040 (Cash, Brokerage and Roth all at 0; IRA at $1,679,275):

| step | value | line |
|---|---|---|
| `targetSpend` | 196,402 | `:1234` |
| `+ IRMAA` | 7,094 | |
| `- possibleIncome` (GROSS: SS 67,823 + RMD 94,213) | 162,035 | `:1189` |
| `= additionalSpendNeeded` | **41,461** | `:1235` |
| primary IRA draw, grossed up at 2039's `nominalTaxRate` 0.2356 | 54,239 (= 41,461 / 0.7644) | `:1387` |
| tax pass 1 `totalTax` (incl. IRMAA) | 48,097 | `:1431` |
| `netSpendable = possibleIncome - totalTax` | 168,178 | `:1473` |
| **`gap = targetSpend - netSpendable`** | **28,224** | `:1474` |
| gap fill: Brokerage 40 / Cash 60, then Roth. All three are 0 | draws **0** | `:1542-1556` |
| `residualGap` | 28,224 | `:1591` |
| third pass: **Cash only, then Roth**. Brokerage deliberately excluded (cap-gains spiral, `:1598-1602`) | draws **0** | `:1614-1626` |
| forced-IRA convergence loop, gate `yr.isBracketStrategy && !yr.isACAStrategy` | **SKIPPED** | `:1651` |
...
The exclusion is deliberate and documented (`:1645-1650`: *"Excluded: strict ACA (subsidy cliff),
...
`yr.targetSpend = ... : Math.min(sim.spendGoal, yr.goalLimit)` (`:1234`) never fires in this run,
...
- **Sign convention.** `Shortfall: Math.min(0, netIncome - sim.spendGoal)` (`:1761`) — negative *is*
  ...
  dollar (`:2152`). No sign error, no double count.
- **The success test measures the right thing.** `netIncome < targetSpend * 0.99` (`:2206`) is
...
**Latent, not live:** `totals.spend += yr.targetSpend + yr.surplus.Shortfall` (`:2152`) mixes a
...
| unknown string -> baseline `else` (`:1400`) | **false** | **-304,331** | **893,920** | 0 |

`gk` and any unrecognized strategy fall through to the baseline `else` (`:1400`) and inherit this.
...
Widen the gate at `:1651` from `yr.isBracketStrategy && !yr.isACAStrategy` to
...
(`isBracketStrategy`) silently excludes everything added later. The comment at `:1645-1650` states a
...
against pre-tax income is what fails. Suite context: 185 tests green with this defect live, because
