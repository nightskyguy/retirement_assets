...
| `rbg_playback_harness.js` | node, **multi-process** | Does the cheap risk-based spend rule (`spendRule: 'rbg'`, a rails table read once a year) follow the same spending as re-solving the chance of success every year along one real historical sequence? Walks five households through six historical starts, exact against cheap, with and without Never above plan (`--ceiling`), beside the plan with no rule and with GK-style. A few minutes on 5 processes; `--quick` is one household and two starts; `--from <dir>` re-prints a run. | [RBG_RULE_VALIDATION.md](RBG_RULE_VALIDATION.md) | 11.189d (2026-09-19) |
| `rbg_grid_harness.js` | node, **multi-process** | What the risk-based spend rule delivers at several Target / Raise / Cut / Cut-returns-to sets - the five presets, the prefilled Custom included, plus a cut 25/35/50 x target 70/80/90 grid - on the households most likely to be cut, through five historical starts and under 400 Monte Carlo paths, beside no rule and GK-style. One rails job per household solves every set at once. About 10 minutes on 4 processes; `--quick` is one household. | [RBG_RULE_THRESHOLDS.md](RBG_RULE_THRESHOLDS.md) | 11.189d (2026-09-19) |
| `rails_parallel_bench.js` | node, **worker_threads** | How much faster the rails job gets with its solved years dealt across 1/2/4/8/16 threads (each year resumes from the spine and needs no other year), whether the merged answers are identical, and what that projects to on the audience's 2-4 core laptops. The measurement behind the P132f server decision. A few minutes. | task_plan `P132f` | 11.189d (2026-09-19) |
| `rails_precision_harness.js` | node, **multi-process** | Is the live rails solve steady at the settings a person would run, can its 99% and 100% raise rails be reached, is every 3 years close enough, and what do the solve and an After-Tax Spend answer cost? Per-path pools (each path's own wealth and spending threshold) for precision against path count, the real job seed after seed, an every-year solve for cadence, and an idle-process timing pass. About 50 minutes on 16 processes (`JOBS=`); `--quick` is a 2-minute smoke test, `--timing-only` re-times the solver in about 3 minutes, and `--from <dir> --no-timing` re-prints a finished run in seconds. `USER_PLAN=` adds a household from a file kept outside the repository. | [RISK_BASED_RAILS_PRECISION.md](RISK_BASED_RAILS_PRECISION.md) | 11.1857 (2026-09-16); `--timing-only` and `--quick` on 11.1859 |
| `medicare_escalation_harness.js` | node, **no engine** | Which growth model should carry Medicare premium and IRMAA surcharge dollars forward? Fits four models to the Part B standard premium 2003-2026 over trailing 5, 10 and 20 year windows, then scores them on the 30-year premium stream against a benchmark built from the Trustees' published path, then tests whether that ranking survives a different long-run assumption. The only harness here that loads neither the engine nor a household: its subject is a data series. Runs in under a second. | [MEDICARE_ESCALATION.md](MEDICARE_ESCALATION.md) | 2026-09-24 |
| `medicare_delta_harness.js` | node | What switching Medicare to the two-phase model costs each household, and three checks that the switch is wired rather than merely present: a flat override must reproduce compound interest (so the old behavior stays reachable), the shipped model's per-household delta, and a zero override must hold premiums at anchor-year dollars. Byte identity is the WRONG test for a deliberate behavior change, and a delta harness that measures nothing looks exactly like one measuring dead plumbing - parts 1 and 3 exist to tell those apart. Seconds. | [MEDICARE_ESCALATION.md](MEDICARE_ESCALATION.md) | 11.1917 (2026-09-24) |
| `identity_harness.js` | node | A tool, not a study: does a refactor change any number a run produces? Walks `simulate()` field by field, both optimizers, `breakEvenHeirsRate`, `suggestSustainableSpend`, the first eight sweep variations and three Monte Carlo modes with the stress pass, over all 19 households: 771,287 values in 13 seconds. `--out <file>` records a snapshot, `--check <file>` re-runs and names the first values that moved, with the year and field. Path arrays are folded to count, sum, index-weighted sum and extremes, so a REORDERED path still shows. Non-finite values are tagged rather than written as `null`, which is what `JSON.stringify` would do to the `Infinity` a bracket ceiling legitimately carries. | *(pass/fail, no report)* | 11.1917 (2026-09-25) |
...
`medicare_escalation_harness.js` is the one exception, marked **no engine** in the table above: its
subject is a published data series rather than a plan, so it loads no engine and no household.
