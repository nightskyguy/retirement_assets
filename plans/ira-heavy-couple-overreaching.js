'use strict';
/**
 * IRA-heavy couple, spending beyond the portfolio
 *
 * A California couple with $4.2M of IRA who spend $292k a year and run out of money before the plan ends.
 *
 * A PLAN CARD, not just a plan. `notes.summary` is the only line a person ever sees - loading this
 * plan writes it into the tool's own Notes box. Everything else in `notes` is for whoever is
 * choosing a household to measure on, and answers the question that saves a wasted study: not
 * "what is this plan" but "what can this plan NOT show".
 *
 * `notes.viability` is MEASURED, by re-running the plan, not asserted. See plans/README.md.
 */

const PLAN = {
    id: "ira-heavy-couple-overreaching",
    title: "IRA-heavy couple, spending beyond the portfolio",
    notes: {
        summary: "A California couple with $4.2M of IRA who spend $292k a year and run out of money before the plan ends.",
        exercises: [
            "what a scorer does with a plan that fails: every wealth metric on it is a comparison between two failures",
            "the third-pass forced IRA draw, which this plan reaches early and often",
        ],
        cannotShow: [
            "ANY ranking question. It is here as the control that must be excluded, not as a household to measure on",
        ],
        viability: {
            funded: "23/33",
            fundsEveryYear: false,
            endingIRA: 0,
            peakIRAYear: 0,
            acaBreachYears: 0
        },
        origin: "the `defaults3x` mix at 6% spend. **Measured failing**, which is the point of keeping it: the same mix at the grid's 8% rate is why 189 of 270 households were skipped in one study.",
    },
    inputs: {
        STATEname: "CA",
        nYears: 20,
        birthyear1: 1962,
        birthmonth1: 6,
        die1: 92,
        birthyear2: 1964,
        birthmonth2: 3,
        die2: 94,
        hasSpouse: true,
        ss1: 45000,
        ss1Age: 70,
        ss2: 24000,
        ss2Age: 67,
        pensionAnnual: 0,
        pensionStartAge: 0,
        survivorPct: 0,
        pensionCola: false,
        spendChange: 0,
        iraBaseGoal: 0,
        inflation: 0.025,
        cpi: 0.025,
        growth: 0.06,
        cashYield: 0.03,
        dividendRate: 0.02,
        ssFailYear: 2099,
        ssFailPct: 1,
        convertExcessToRoth: true,
        propWithdraw: 0.1,
        iraWithdrawPct: 0.06,
        extraConversionAmount: 0,
        fundConversionWithCash: false,
        startAge: 64,
        startInYear: 2026,
        dividendReinvest: true,
        gkGuard: 0.2,
        gkAdjPct: 0.1,
        cycleLTCGTarget: 0.15,
        qcdHHMax: 0,
        qcdMode: "asneeded",
        computeOC: false,
        IRA1: 3000000,
        IRA2: 1200000,
        Roth: 150000,
        Roth2: 60000,
        Brokerage: 300000,
        BrokerageBasis: 150000,
        Cash: 150000,
        spendGoal: 291600,
        strategy: "propwd"
    },
};

// Dual-mode export, the repo's classic-script contract: `require()` in node, `window.Plans` in the
// browser, so the same file can back a harness and a page load without a build step.
if (typeof module !== 'undefined' && module.exports) module.exports = PLAN;
if (typeof window !== 'undefined') (window.Plans = window.Plans || {})[PLAN.id] = PLAN;
