...
| `.githooks/md-html-scan.js` | Blocks raw HTML in markdown that hides the rest of the file. A bare `<select>` cost 4,430 lines of VS Code preview in `task_plan.md`; GitHub sanitizes the tag away, so only an unsanitized preview shows the break. Denylists the 11 elements MEASURED to hide following content (`select`, `textarea`, `title`, `style`, `script`, `noscript`, `iframe`, `template`, `details`, `object`, `dialog`); `<a>`/`<b>`/`<li>`/`<option>`/`<table>` pass. The list is re-derived in a browser, never reasoned - see the header comment. Run standalone on any file: `node .githooks/md-html-scan.js <file.md>`. |
